...
export function ProductExperience({ product }: { product: Product }) {
  ...
  return (
    <div lang={language} dir={isRtl ? "rtl" : "ltr"} className="min-h-screen bg-white text-[#0F1822]">
      ...
      <main id="top">
        <section className="bg-[#050709] px-3 pb-4 pt-3 text-white sm:px-5 sm:pb-5 sm:pt-4 lg:px-8">
          <div className="mx-auto flex max-w-[1440px] items-center gap-2 overflow-x-auto pb-3 text-xs text-[#73787C] hide-scrollbar">
          ...
          <div className="mx-auto grid max-w-[1440px] overflow-hidden border border-white/10 bg-[#0B1015] lg:grid-cols-[1.14fr_0.86fr]">
            <div className="min-w-0 p-2.5 sm:p-4">
              <div className="relative h-[min(64dvh,460px)] min-h-[320px] overflow-hidden bg-[#151A1D] sm:h-[min(68dvh,600px)] sm:min-h-0 lg:h-[clamp(520px,72vh,740px)]">
                ...
                <div className="pointer-events-none absolute inset-4 border border-white/15 sm:inset-6" aria-hidden="true" />
                <span className="absolute start-4 top-4 bg-[#FFDA01] px-3 py-2 sm:start-6 sm:top-6 sm:px-4 sm:py-3 text-[10px] font-bold tracking-[0.12em] text-[#0F1822]">{product.code}</span>
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="min-w-0 p-5 sm:p-8 lg:p-10 xl:p-12">
              <div>
                ...
                <h1 className="mt-5 break-words text-balance text-4xl font-semibold leading-[0.98] tracking-[-0.05em] sm:text-5xl xl:text-6xl">{name}</h1>
                <p className="mt-4 max-w-xl text-base leading-7 text-[#A3A7AA]">{description}</p>
                <div className="mt-6 flex flex-wrap items-end justify-between gap-4 border-y border-white/10 py-4">
                  <div>
                ...
                <div className="mt-6 grid grid-cols-1 gap-px border border-white/10 bg-white/10 sm:grid-cols-2">
                  {product.specs.slice(0, 4).map(([label, value]) => <div key={label} className="min-w-0 bg-[#0B1015] p-4"><p className="break-words text-[9px] font-bold uppercase tracking-[0.16em] text-[#73787C]">{label}</p><p className="mt-1.5 break-words text-sm font-semibold text-white">{value}</p></div>)}
                </div>

                <div className="mt-5 flex items-start gap-3 border-s-2 border-[#FFDA01] ps-4"><Check size={17} className="mt-0.5 shrink-0 text-[#FFDA01]" /><p className="text-sm leading-6 text-[#A3A7AA]">{localize(language, "Initial price, availability, and delivery timing are confirmed with the Kiswani team before final approval.", "السعر الأولي والتوفر وموعد التسليم يتم تأكيدها مع فريق كسواني قبل اعتماد الطلب النهائي.", "מחיר ראשוני, זמינות וזמן אספקה יאושרו עם צוות Kiswani לפני אישור סופי.")}</p></div>
              </div>
                  ...
                  <div className="inline-flex items-center border border-white/20 bg-[#151A1D]">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!isNaN(val) && val > 0) setQuantity(Math.min(999, val));
                      }}
              ...
              <div className="mt-5 grid gap-3">
                <motion.button type="button" onClick={() => add(product, quantity)} whileTap={{ scale: 0.985 }} className="inline-flex min-h-13 items-center justify-center gap-3 bg-[#FFDA01] px-5 text-sm font-bold text-[#0F1822] transition-colors hover:bg-[#FFD100]"><ShoppingBag size={18} />{localize(language, "Add to project cart", "أضف إلى سلة الطلب", "הוספה לסל הפרויקט")}</motion.button>
        ...
        <section className="bg-[#F4F2ED] px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
          <div className="mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-16">
            <div>
              ...
              <h2 className="mt-4 text-3xl font-semibold leading-[1.02] tracking-[-0.04em] sm:text-4xl">{isArabic ? "تفاصيل دقيقة لقرار واثق." : "Precise details for a confident decision."}</h2>
              <p className="mt-4 max-w-lg leading-7 text-[#73787C]">{isArabic ? "راجع المواصفات الأساسية وحمّل ورقة البيانات لمشاركتها مع المصمم أو المقاول." : "Review the essential specifications and download the data sheet to share with your designer or contractor."}</p>
              <a href={`/downloads/${product.code}.pdf`} download className="mt-6 inline-flex min-h-13 items-center gap-3 bg-[#0F1822] px-6 text-sm font-bold text-white"><FileText size={17} className="text-[#FFDA01]" />{isArabic ? "تحميل ملف PDF" : "Download specification PDF"}</a>
            </div>

            <div className="border-t border-[#A3A7AA]">
              {product.specs.map(([label, value], index) => <div key={label} className="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-x-4 gap-y-1 border-b border-[#CCCFCE] py-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center"><span className="text-[10px] font-semibold text-[#A3A7AA]">0{index + 1}</span><span className="break-words text-sm font-medium text-[#50555B]">{label}</span><strong className="col-start-2 break-words text-sm text-[#0F1822] sm:col-auto sm:text-end">{value}</strong></div>)}
              <div className="mt-6 grid gap-4 bg-white p-5 md:grid-cols-3">
        ...
        {related.length > 0 && <section className="bg-white px-4 py-14 sm:px-6 sm:py-20 lg:px-8"><div className="mx-auto max-w-[1440px]"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#73787C]">{isArabic ? "قد يناسبك أيضاً" : "RELATED LIGHTING"}</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">{isArabic ? "من المجموعة نفسها" : "From the same collection"}</h2></div><a href={`/collections/${product.categorySlug}`} className="inline-flex items-center gap-2 text-sm font-bold">{isArabic ? "عرض المجموعة" : "View collection"}<ArrowRight size={16} className={isArabic ? "rotate-180" : ""} /></a></div><div className="mt-8 grid gap-4 md:grid-cols-3">{related.map((item) => <motion.a key={item.code} href={`/products/${getProductSlug(item)}`} whileHover={{ y: -7 }} className="group block bg-[#F4F2ED]"><div className="relative aspect-[4/3] overflow-hidden"><Image unoptimized src={item.image} alt={isArabic ? item.arabic : item.name} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.035]" /><span className="absolute end-4 top-4 flex h-11 w-11 items-center justify-center bg-[#FFDA01]"><ArrowUpRight size={16} /></span></div><div className="p-5"><p className="text-[10px] font-semibold tracking-[0.15em] text-[#73787C]">{item.code}</p><h3 className="mt-2 break-words text-xl font-semibold tracking-[-0.03em] sm:text-2xl">{isArabic ? item.arabic : item.name}</h3></div></motion.a>)}</div></div></section>}

        <section id="contact" className="bg-[#FFDA01] px-4 py-14 sm:px-6 sm:py-20 lg:px-8"><div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#50555B]">{isArabic ? "دعم المشاريع" : "PROJECT SUPPORT"}</p><h2 className="mt-3 max-w-4xl text-3xl font-semibold leading-[1] tracking-[-0.04em] sm:text-5xl">{isArabic ? "هل تريد هذه القطعة ضمن مخطط إنارة متكامل؟" : "Want this fixture in a complete lighting plan?"}</h2></div><a href={`https://wa.me/970599671209?text=${message}`} target="_blank" rel="noreferrer" className="inline-flex min-h-14 items-center justify-center gap-3 bg-[#0F1822] px-7 text-sm font-bold text-white">{isArabic ? "تحدث مع مستشار" : "Talk to an advisor"}<ArrowUpRight size={17} /></a></div></section>
      </main>
